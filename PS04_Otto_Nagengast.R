...
## All of the following is Professor Kim's code. ##
...
## My code is below, interlaced with Professor Kim's code. 
## My code is after the sections that begin with "# FILL THIS IN:"  
...
for(i in 1:n_sim) {
  ...
  # Log the number of runs of the loop # 
  if(i %% 100 == 0){
    print(i)
  }
}
...
# MSE # 
truth <- truth %>% 
  mutate(resid = (y - y_hat)^2) 

truth %>%   
  summarise(MSE = mean(resid)) 

# Avg Bias of f_hat #
truth <- truth %>% 
  mutate(bias = (f_x - y_hat)^2) 
truth %>% 
