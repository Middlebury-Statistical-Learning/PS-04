...
for(i in 1:n_sim) {
  #Generate random sample to use as training data
  train_data <- generate_sample(f, n_sim = 100, sigma = sigma)
  #select only the appropriate columns
  train_data <- train_data %>%
    select(x, y)
  #refit the linear model to the training data
  model <- lm(y ~ x, data = train_data)
  train_data_augmented <- model %>%
    augment() %>%
    tbl_df()
  ...
  prediction <- model %>%
    ...
    tbl_df() %>%
    .[[".fitted"]]
  #push prediction to vector of predictions
...
MSE <- mean(sum((truth$y_hat - truth$y)^2))

avg_bias_f_hat <- mean(sum((truth$f_x - truth$y_hat)^2))

variance <- var(truth$y_hat)

sigma_sq <- (sd(truth$eps))^2
