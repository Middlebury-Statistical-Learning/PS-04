...
# Didn't know where the residual values were suppose to be


variance_of_fhat = var(truth$y_hat)

sigma_squared = truth %>% mutate(sigma_squared = eps^2)

avg_bias_of_f_hat = truth %>% summarise(fhat_bias = mean(y_hat)^2)

mse_values = avg_bias_of_f_hat^2 + variance_of_fhat + sigma_squared
