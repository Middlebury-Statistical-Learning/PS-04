...
for(i in 1:n_sim) {
  train_data_new <- generate_sample(train_data, n_sim, sigma = sigma)
  model <- lm(y ~ x, data = train_data_new)
  predictions[i] <- predict(model, test_data)
  rm(train_data_new, model)
  if(i %% 100 == 0){
    print(i)
  }
}
...
Avg_Bias <- mean((truth$f_x - truth$y)^2)
print(Avg_Bias)

Var_fhat <- var(truth$f_x)
print(Var_fhat)

sigma_squared <- sigma^2
print(sigma_squared)

tot <- sigma_squared + Var_fhat + Avg_Bias
print(tot)
print(MSE)
