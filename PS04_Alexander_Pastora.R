...
for(i in 1:n_sim) {
  # Retrain the training data. (i.e Retrain the model)
  train_data <- generate_sample(f, n_sim = 100, sigma = sigma)
  train_data <- train_data %>%
    select(x, y)
...
sum = 0
for(i in 1:n_sim){
  sum = sum + ((truth$y[i]-truth$y_hat[i])^2)
  if(i %% 100 == 0){
    print(i)
  }}
MSE <- sum / n_sim
...
sum = 0
for(i in 1:n_sim){
  sum = sum + ((truth$y[i]-truth$y_hat[i]))
  if(i %% 100 == 0){
    print(i)
  }}
bias_squared <- (sum / n_sim)^2
bias_squared

# Variance of f_hat
Variance_Predicted <- var(truth$y_hat)
Variance_Predicted

# sigma^2
sigma_squared <-var(truth$eps)
sigma_squared

# Add it up
MSE_approximation <- bias_squared + Variance_Predicted + sigma_squared

# Even though the values are off by .0001, the MSE_approximation and MSE
# are equal.
MSE_approximation
MSE
