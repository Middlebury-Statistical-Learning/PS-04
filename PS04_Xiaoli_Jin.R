...
for(i in 1:n_sim) {
  # FILL THIS IN:
  train_data <- generate_sample(f,100, sigma = sigma)
...
# FILL THIS IN:
truth <- truth %>% 
  mutate(Squared_error = (y - y_hat) * (y - y_hat)) %>% 
  mutate(Bias_f_hat = f_x - y_hat) 


a = dplyr::summarise(truth,MSE = mean(Squared_error)) %>% 
  .[["MSE"]]
b = dplyr::summarise(truth, Avg_bias_f_hat_squared = mean(Bias_f_hat)*mean(Bias_f_hat)) %>% 
  .[["Avg_bias_f_hat_squared"]]
c = dplyr::summarise(truth, Var_f_hat = var(y_hat)) %>% 
  .[["Var_f_hat"]]
d = dplyr::summarise(truth, Var_sigma = var(y)) %>% 
  .[["Var_sigma"]]
