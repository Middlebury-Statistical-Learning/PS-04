...
for(i in 1:n_sim) {
  # FILL THIS IN:
  #creat the samples (test is constant at 0.95)
  train_data <- generate_sample(f, n_sim = 100, sigma = sigma)
  test_data <- data_frame(x = 0.95)
...
#Calculate MSE
MSE <-  mean(sum((truth["y"] - truth["y_hat"])^2))

#Calculate (Avg Bias of f_hat)^2
avg_bias_f_hat_sqrd <- mean(sum((truth["f_x"] - truth["y_hat"])^2))

#Calculate Variance of f_hat
f_hat_var <- var(truth["y_hat"])

#Calculate sigma^2 (from the data)
sig_sqrd1 <-  sd(truth$eps)^2

#Calculate sigma^2 (from what we are given)
sig_sqrd2 <- sigma^2
