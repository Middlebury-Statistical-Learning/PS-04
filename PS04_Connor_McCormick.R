...
for(i in 1:n_sim) {
  #sampling
  training <- generate_sample(f, n_sim = 100, sigma = sigma) 

  #model
  model <- lm(y ~ x, data = training)
  train_data_augmented <- model %>%
    augment() %>%
    tbl_df()
...
#(Avg Bias of f_hat)^2
avgbias<- truth %>%
  summarise(bias=mean((y_hat-y)/y))%>%
  mutate(avgbiassq = (bias^2)) %>%
  select(avgbiassq)

#variance
vari <- truth %>%
  mutate(variance = var(y_hat)) %>%
  summarise(var = mean(variance))

#sigma^2
sigsq <- truth %>%
  mutate(mean = mean(y-y_hat)) %>%
  mutate(meansig = mean((y_hat-mean)^2)) %>%
  summarise(sigsq = mean(meansig))
