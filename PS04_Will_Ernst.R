...
for(i in 1:n_sim) {
  ...
  #build a model
  temp_model <- lm(y ~ x, data = ps_train)
  #predict for point x0
...
output <- truth %>%
  #compute new cols
  mutate(mse = mean((y_hat-y)^2), bias = f_x-y_hat) %>%
  #output necessary data
  summarise(avg_mse = mean(mse), avg_bias_sq = mean(bias)^2, variance = var(y_hat), simga_sq = sd(eps)^2)
